package org.flowplayer.pseudostreaming.net
{
    ...
    import org.flowplayer.pseudostreaming.Decipher;
    ...
    public class ByteRangeNetStream extends NetStream
	{
		...
		private function onIOError(event:IOErrorEvent):void {
			...
			dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false, {code:"NetStream.Play.Failed", level:"error", message: "IO error: " + event.text})); 
		}
		
		private function onSecurityError(event:SecurityErrorEvent):void {
			log.debug("Security error has occured " + event.text);
			dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false, {code:"NetStream.Play.Failed", level:"error", message: "Security error：" + event.text})); 
		}
		
		private function onError(event:HttpErrorEvent):void {
			log.debug("An error has occured " + event.text);
			dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false, {code:"NetStream.Play.Failed", level:"error", message: "Error: " + event.text})); 
		}
		
		private function onTimeoutError(event:HttpErrorEvent):void {
			log.debug("Timeout error has occured " + event.text);
			dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false, {code:"NetStream.Play.Timeout", level:"error", message: event.text})); 
		}
		...
		private function onData(event:HttpDataEvent):void {
            var buffer:ByteArray = event.bytes;
            if (_decrypt){
               try {
                    buffer = _decrypt.Analyse(buffer);
                }
                catch (e:String)
                {
                    dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,false,false, {code:e, level:"error"})); 
                    return ;
                }
            }
            if (buffer != null) {
