        ...
        private function _onNetStatus(event:NetStatusEvent):void {
            ...
            if (event.info.code == "NetStream.Buffer.Empty") {
                ...
                if (--_attempts > 0) {
                    ...
                    clip.dispatchError(ClipError.STREAM_NOT_FOUND, event.info.message);
                    closeStreamAndConnection(netStream);
                }
            } else if (event.info.code == "NetStream.Play.ErrorFormat") {
                dispatchPlayEvent(ClipEventType.BUFFER_STOP);
                clip.dispatchError(ClipError.STREAM_ERR_FORMAT, event.info.message);
                closeStreamAndConnection(netStream);
            } else if (event.info.code == "NetStream.Play.Timeout") {
                clip.dispatchError(ClipError.STREAM_TIMEOUT, event.info.message); 
                closeStreamAndConnection(netStream);
            }
            else if (event.info.code == "NetStream.Play.Failed") {
                clip.dispatchError(ClipError.SERVER_FAILED, event.info.message);

//                clip.dispatchError(new ClipError(ClipEventType.ERROR, 500, event.info.message), event.info.code); 
                closeStreamAndConnection(netStream);               
            }
            onNetStatus(event);
        }
