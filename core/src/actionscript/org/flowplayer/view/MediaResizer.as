...
package org.flowplayer.view {
	...
	internal class MediaResizer {
		...
		public function resizeTo(sizingOption:MediaSize, force:Boolean = false):Boolean {
            ...
            } else if (sizingOption == MediaSize.SCALE_TO_16_9) {
                resized = resized = scalePreset(2.21, 1.24);;
            } else if (sizingOption == MediaSize.SCALE_TO_4_3) {
                resized = scalePreset(2, 1.5);
            } else if (sizingOption == MediaSize.SCALE_TO_1B) {
                resized = scaleFigure(1);
            } else if (sizingOption == MediaSize.SCALE_TO_15B) {
                resized = scaleFigure(1.5);
            } else if (sizingOption == MediaSize.SCALE_TO_2B) {
                resized = scaleFigure(2);
            } else if (sizingOption == MediaSize.SCALE_TO_05B) {
                resized = scaleFigure(0.5);
            }
			_currentSizingOption = sizingOption;
			return resized;
		}
        
        private function scalePreset (w:Number, h:Number):Boolean {
            var dce:Number = _clip.width * _clip.width + _clip.height * _clip.height;
            dce = Math.sqrt(dce);
            var widthPreset:Number = dce * w;
            var heightPreset:Number = dce * h;         
            
            var t1:Number = widthPreset /_maxWidth;
            var t2:Number = heightPreset /_maxHeight; 
            if (t1 > 1 || t2 > 1) { 
                widthPreset = Math.floor(widthPreset / (t1 > t2 ? t1 : t2)); 
                heightPreset = Math.floor(heightPreset / (t1 > t2 ? t1 : t2)); 
            }
            resize(widthPreset, heightPreset);
            return true;
        }
        
        private function scaleFigure(scalingFactor:Number):Boolean {
            scale(scalingFactor);
            return true;
        }
