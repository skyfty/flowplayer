...
//                    throw new Error("Plugin " + loadables[i] + " not available");
	...
	//                    plugin is ProviderModel && ProviderModel(plugin).pluginObject is Plugin;
		...
		public function showError(message:String):void {
            ...
            /*var info:DisplayPluginModel = _pluginRegistry.getPlugin("info") as DisplayPluginModel;
            info.invokeMethod("show", [ message ]);
            
           
            var errorHideTimer:Timer = new Timer(10000, 1);
            errorHideTimer.addEventListener(TimerEvent.TIMER_COMPLETE, function(event:TimerEvent = null):void {
                if (info) {
                    if (_animationEngine) {
                        _animationEngine.fadeOut(_error, 1000, function():void { info.visible = false;; });
                    } else {
                        info.visible = false;
                    }
                }
		...
		private function doHandleError(message:String, throwError:Boolean = false):void {
		...
		private function addPluginsToPanel(_pluginRegistry:PluginRegistry):void {
			for each (var pluginObj:Object in _pluginRegistry.plugins) {
				if (pluginObj is DisplayPluginModel) {
                    ...
                    if (model.name == "quickmenu") {
                        _quickmenuModel = model;
                    }
				}
			}
			if (_controlsModel) {
        ...
        private function isQuickMenuAlwaysAutoHide():Boolean {
            if (!_quickmenuModel) return false;
            var quickmenu:Object = _quickmenuModel.getDisplayObject();
            //#583 this seems to handle the fullscreenOnly property better
            return  !quickmenu.getAutoHide().fullscreenOnly;
        }
